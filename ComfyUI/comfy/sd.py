...
_CLIP_MISSING_GUIDANCE = {
    "Flux": (
        "This is a Flux diffusion model without bundled text encoders.\n"
        "Use split loaders instead of CheckpointLoaderSimple:\n"
        "  1. Put the UNet in models/diffusion_models/ -> \"Load Diffusion Model\" (UNETLoader)\n"
        "  2. Put clip-l and t5-xxl in models/text_encoders/ -> \"DualCLIPLoader\" (type: flux)\n"
        "  3. Put ae.safetensors in models/vae/ -> \"Load VAE\" (VAELoader)"
    ),
    "FluxSchnell": (
        "This is a Flux Schnell diffusion model without bundled text encoders.\n"
        "Use UNETLoader + DualCLIPLoader (type: flux) + VAELoader instead of CheckpointLoaderSimple."
    ),
    "FluxInpaint": (
        "This is a Flux inpaint diffusion model without bundled text encoders.\n"
        "Use UNETLoader + DualCLIPLoader (type: flux) + VAELoader instead of CheckpointLoaderSimple."
    ),
    "Flux2": (
        "This is a Flux 2 diffusion model without bundled text encoders.\n"
        "Use UNETLoader + DualCLIPLoader (type: flux2) + VAELoader instead of CheckpointLoaderSimple."
    ),
    "SD3": (
        "This SD3 checkpoint has no text encoder weights.\n"
        "Use DualCLIPLoader (type: sd3) with clip-l, clip-g, and/or t5-xxl from models/text_encoders/."
    ),
    "SDXL": (
        "This SDXL checkpoint has no CLIP weights.\n"
        "Download a full SDXL checkpoint, or use DualCLIPLoader (type: sdxl) with clip-l and clip-g in models/text_encoders/."
    ),
    "SDXLRefiner": (
        "This SDXL refiner checkpoint has no CLIP weights.\n"
        "Load CLIP separately with DualCLIPLoader (type: sdxl) or use a full SDXL checkpoint."
    ),
    "HunyuanVideo": (
        "This Hunyuan Video model has no bundled text encoder.\n"
        "Use UNETLoader + DualCLIPLoader (type: hunyuan_video) + VAELoader."
    ),
    "Wan": (
        "This Wan model has no bundled text encoder.\n"
        "Use UNETLoader + CLIPLoader (type: wan) + VAELoader."
    ),
}

def checkpoint_clip_missing_hint(model_patcher, ckpt_path):
    filename = os.path.basename(ckpt_path)
    message = [
        "ERROR: clip input is invalid: None",
        "",
        f"The checkpoint '{filename}' loaded a diffusion model but no CLIP/text encoder was found.",
        "CLIPTextEncode needs a valid CLIP connection from your loader nodes.",
    ]

    if "lora" in filename.lower():
        message.append("")
        message.append(
            "HINT: This file looks like a LoRA. Move it to models/loras/ and load it with a LoRA loader node, not CheckpointLoaderSimple."
        )
        return "\n".join(message)
    ...
    if model_config is not None:
        guidance = _CLIP_MISSING_GUIDANCE.get(model_config.__class__.__name__)
    ...
    message.extend([
        "",
        "Common causes:",
        "  - The file is diffusion/UNet-only (no text encoders bundled)",
        "  - The checkpoint download is incomplete or corrupt",
        "  - A LoRA or partial weight file was placed in models/checkpoints/",
        "",
        "How to fix:",
        "  - SD 1.5 / SDXL: use a full .safetensors checkpoint with text encoders included",
        "  - Flux / SD3 / split models: use UNETLoader + DualCLIPLoader + VAELoader",
        "  - Wire the loader CLIP output to CLIPTextEncode (not MODEL or VAE)",
        "  - Place text encoders in models/text_encoders/ (models/clip/ also works)",
    ])
    return "\n".join(message)

def ensure_checkpoint_clip(model_patcher, clip, ckpt_path):
    if clip is None:
        raise RuntimeError(checkpoint_clip_missing_hint(model_patcher, ckpt_path))
    return clip
